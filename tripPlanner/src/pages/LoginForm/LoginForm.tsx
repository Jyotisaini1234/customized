...
const LoginForm: React.FC = () => {
  ...
  const { data: companyInfoData, error: companyInfoError, isLoading: isLoadingCompanyInfo, refetch: refetchCompanyInfo } = 
    useGetUserCompanyInfoQuery(getEmailForQuery(), { 
      skip: !shouldFetchCompanyInfo || !getEmailForQuery(), 
  ...
  const handleSuccessfulLogin = useCallback(async () => {
    ...
    try {
      ...
      if (token && refreshToken) {
        ...
        try {
          ...
          console.error('URL construction failed, using fallback:', urlError);
          finalUrl = redirectUrl;
        }
    ...
    redirectInProgress, token, refreshToken, user, getRedirectUrl, username, 
    companyInfoData, companyInfoError, shouldFetchCompanyInfo, refetchCompanyInfo
  ]);
  ...
  useEffect(() => {
    if (isAuthenticated && token && refreshToken && !redirectInProgress && hasAttemptedLogin) {
      console.log('All conditions met, calling handleSuccessfulLogin');
      handleSuccessfulLogin();
    }
  }, [
    isAuthenticated, token, refreshToken, user?.email, redirectInProgress, 
    hasAttemptedLogin, handleSuccessfulLogin, isLoadingCompanyInfo
  ]);

  useEffect(() => {
    const checkRegistrationSuccess = async () => {
      const registrationSuccess = await getFromDB(STORES.plannerData, 'registrationSuccess', false);
      if (registrationSuccess) {
        setSuccess('Registration successful! Please login with your credentials.');
        await saveToDB(STORES.plannerData, 'registrationSuccess', false);
      }
    };
