...
import {loginUser,registerUser,logoutUser, refreshTokenAsync,updateActivity, checkSession, initializeAuth, forceLogout,clearError, setCompanyInfo,setRegistrationSuccess, updateFromUrlParams} from '../store/slices/authSlice.ts';
...
export const useAuth = () => {
  ...
  const setRegSuccess = useCallback((success: boolean) => {
    dispatch(setRegistrationSuccess(success));
  }, [dispatch]);

  const handleUrlParams = useCallback((params: {
    email?: string;
    companyName?: string;
    logoPath?: string;
    userName?: string;
    ...
    dispatch(updateFromUrlParams(params));
  }, [dispatch]);
  ...
  return {
    ...
    updateCompanyInfo,
    setRegistrationSuccess: setRegSuccess,
    handleUrlParams,
  };
};