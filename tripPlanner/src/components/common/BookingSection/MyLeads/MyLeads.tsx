...
const MyLeads: React.FC = () => {
  ...
  const getStatusChip = (status: string, isTourTransfer: boolean = false) => {
    // Convert to lowercase for comparison but keep original casing logic
  ...
  const transformBookingToLead = (booking: any) => {
    ...
    // Fix: Use the actual status from booking data, convert to lowercase for consistency
    ...
    return {
      ...
      // Fix: Use only the actual status from database, no special handling for tour transfers
      status: actualStatus,
  ...
  const handleEditLead = (lead: Lead) => {
    if (lead.isBooking) {
      alert('Tour bookings cannot be edited from this interface.');
      return;
    }
          ...
          {sortedData && sortedData.length > 0 ? (
            sortedData.map((lead) => (
              <TableRow key={`${lead.isBooking ? 'booking' : 'lead'}-${lead.id}`}>
                <TableCell>
                  <Button  sx={{   textTransform: 'none', fontWeight: 'bold',  padding: 0,  minWidth: 'auto',  textAlign: 'left',   color: '#0275d8'}}   onClick={() => handleLeadDetails(lead)}>{lead.referenceId}
                  </Button>
                </TableCell>
                <TableCell>
                  <Button onClick={() => handleEditLead(lead)}  disabled={lead.isBooking} sx={{  minWidth: 'auto',  padding: '4px 8px',   color: lead.isBooking ? '#ccc' : '#0275d8' }} >
                    <Edit fontSize="small" />
                  </Button>
                </TableCell>
                ...
                <TableCell>
                  <Link   component="button" variant="body2"  onClick={() => handleInvoiceDownload(lead)} sx={{  cursor: 'pointer',   color: '#0275d8',  textDecoration: 'none'  }} >  Invoice </Link>
                </TableCell>
                <TableCell>
                  <Link component="button" variant="body2"   onClick={() => handleInvoiceDownload(lead)} sx={{ cursor: 'pointer',  color: '#0275d8',  textDecoration: 'none'   }} >  Itinerary
                  </Link>
                </TableCell>
