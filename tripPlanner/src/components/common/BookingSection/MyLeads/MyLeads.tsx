...
//       items.push({ id: `day-${items.length + 1}`,date: `${day.toString().padStart(2, '0')}-${month} ${year}`,dateObj: new Date(dayDate),hotel: null, transfer: null,  tours: null, meals: null, eventDate: '', });
...
//       //       const matchingIndex = mergedItems.findIndex(item => item.id === existingItem.id);
//       //       if (matchingIndex !== -1) {
//       //         mergedItems[matchingIndex] = {
//       //           ...mergedItems[matchingIndex],
//       //           ...existingItem,
//       //           tours: updatedItems[matchingIndex]?.tours || existingItem.tours
//       //         };
//       //       }
//       //     });
          
//       //     sessionStorage.setItem('updatingtripPlannerItems', JSON.stringify(mergedItems));
//       //   }
//       //   else{
...
//           }
//         }
//         const uniqueHotels = Array.from(new Map(existingHotels.map(h => [h.uniqueId, h])).values());
//         const src = sessionStorage.getItem('src');
//         if (src === 'editLead' || src === 'lead') {
...
//       const itemDataToUpdate = JSON.parse(savedItemData); setPlannerItems(itemDataToUpdate.plannerItems || []); }
//   } else {
//     savedItemData = sessionStorage.getItem('tripPlannerItems');
//     if (savedItemData !== null) { const savedItems = JSON.parse(savedItemData); setPlannerItems(savedItems || []);}}
// }, []);
// useEffect(() => {
//     let src = sessionStorage.getItem('src');
//     if (src === 'lead') {
...
// const srcValue = sessionStorage.getItem('src');
// const editData = JSON.parse(sessionStorage.getItem('editLeadData') || '{}');
// const editingClientData = JSON.parse(sessionStorage.getItem('editingClientData') || '{}');
// const isEditMode = srcValue === 'editLead' ||  srcValue === 'lead' || editData.isEditMode === true ||!!editingClientData.id;
...
//     // const finalBookingRef = isEditMode ? (editingClientData?.id || editingClientData?._id || editData.leadId || editData.bookingRef ||bookingRef) :  bookingRef;
