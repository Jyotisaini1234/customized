...
import { BAKU_REQUIREMENT, BASE_URL } from "../../../../../../utils/ApiConstants.ts";
import { ReadymadeData } from "../../../../../../types/package.types.ts";
import { resetHotelsAndPlannerItems, setCurrentSearchParams, setSessionId, setTripPlannerParams } from "../../../../../../store/slices/tripPlannerSlice.ts";
...
const ReadyMadeSearch: React.FC = () => {
  ...
  useEffect(() => {
    if (packageDetails) {
      ...
      const nightsFromHotel = selectedHotelOption?.hotels?.reduce((total: number, hotel: any) => total + (hotel.nights || 0), 0) || 0;
  ...
  const addRoom = () => {
    const newRoom: Room = { id: Date.now().toString(),adults: 2,cwb: 0,cnb: 0, infants: 0 };
  ...
  //   const sessionId = `readymade_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  //   const searchParams = {
  ...
  //     dispatch(resetHotelsAndPlannerItems());
  //     dispatch(setTripPlannerParams({
 ...
 // Update the handleSearch function in ReadyMadeSearch.tsx

// Complete fixed handleSearch function for ReadyMadeSearch.tsx

const API_BASE_URL = `${BASE_URL}/sightTour`;
...
const handleSearch = async () => {
  ...
  // Generate session ID with readymade prefix
  const sessionId = `readymade_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const searchParams = {
    ...
    preserveOriginalParams: false,
    isHotelSpecific: false,
    searchTimestamp: Date.now(),
    sourcePort: parseInt(window.location.port) || 3000,
    targetPort: 8081
  };

  try {
    // Save search params to backend
    console.log('Saving search params to backend with session ID:', sessionId);
    const searchParamsResponse = await fetch(`${API_BASE_URL}/planner/search-params`, {
      method: 'POST',
      headers: { 
    ...
    if (!searchParamsResponse.ok) {
      const errorText = await searchParamsResponse.text();
      console.error('Backend search params error:', errorText);
      
      // Continue anyway - frontend will work without backend
      console.warn('Continuing without backend session save');
    } else {
      const searchParamsResult = await searchParamsResponse.json();
      console.log('Search params saved successfully:', searchParamsResult);
    }

    // Prepare trip planner data
    const hotels = selectedHotelOption?.hotels || [];
    ...
    console.log('Navigating to planner with data:', tripPlannerData);
    ...
    navigate('/readymade-planner', { state: tripPlannerData });
  } finally {
    setLoading(false);
  }
};
              ...
              <Grid container spacing={2} className='select-form'>
                ...
                <Grid item xs={12} sm={6}>
                  ...
                  <FormControl fullWidth variant="outlined" size="small" className='select-option-2'>
                    <Select value={selectedCity || ''} onChange={(e) => setSelectedCity(e.target.value)}  displayEmpty sx={{ bgcolor: '#f5f5f5', '& .MuiInputBase-root': { height: '2rem', width: '16rem' } }}>
                      {packageDetails?.destinations?.map((destination: string, index: number) => (
                ...
                {rooms.map((room, index) => (
                  <Grid item xs={12} key={room.id} className='person-count'>
                    <Box sx={{ mb: 1, p: 0, borderRadius: '8px' }} className='person-div'>
                      <Typography variant="body1" fontWeight="medium" className='room'>
                        Room {index + 1}
                      </Typography>
                      <Grid container spacing={2} alignItems="center">
                        <Grid item xs={12} sm={3}>
                          ...
                          <FormControl fullWidth size="small">
                            <Select value={room.adults.toString()}    onChange={(e) => updateRoom(room.id, 'adults', Number(e.target.value))}   sx={{ height: '40px' }} >
                        ...
                        <Grid item xs={12} sm={3}>
                          ...
                          <FormControl fullWidth size="small">
                            <Select value={room.cwb.toString()}    onChange={(e) => updateRoom(room.id, 'cwb', Number(e.target.value))} sx={{ height: '40px' }}>
                        ...
                        <Grid item xs={12} sm={3}>
                          ...
                          <FormControl fullWidth size="small">
                            <Select value={room.cnb.toString()}   onChange={(e) => updateRoom(room.id, 'cnb', Number(e.target.value))}   sx={{ height: '40px' }} >
                              {[0, 1, 2, 3, 4].map(num => (<MenuItem key={num} value={num}>{num}</MenuItem>))}  </Select>
                          </FormControl>
                        </Grid>
                        <Grid item xs={12} sm={3}>
                          <Typography variant="body2" sx={{ mb: 0.5 }}>Infant/s<br/>&lt; 2 yrs </Typography>
                          <FormControl fullWidth size="small">
                            <Select value={room.infants.toString()}     onChange={(e) => updateRoom(room.id, 'infants', Number(e.target.value))}   sx={{ height: '40px' }}>
                              {[0, 1, 2, 3, 4].map(num => (<MenuItem key={num} value={num}>{num}</MenuItem>))}
                            </Select>
                          </FormControl>
                        </Grid>
                ...
                <Grid item xs={12}>
                  <Box display="flex" mt={1} className='btn-container'>
                    <Button className='add-another-btn'  onClick={addRoom}  sx={{ backgroundColor: '#6c757d', color:'white',  boxShadow: 'none',     textTransform: 'none',  mr: 2  }}> + Add Another Room </Button>
