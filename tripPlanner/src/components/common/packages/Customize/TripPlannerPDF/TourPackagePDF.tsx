...
//   // Fetch data from database using the booking reference
//   const { data: leadData, error: apiError, isLoading: apiLoading } = useGetLeadByIdQuery(
//     bookingRef || '', 
//     { skip: !bookingRef }
//   );
...
//   useEffect(() => {
//     if (packageData && !isLoading && !error && !pdfGenerated) {
//       const timer = setTimeout(() => {
//         console.log('🚀 Running generatePDF() after full rendering...');
//         generatePDF();
//       }, 1000);

//       return () => clearTimeout(timer);
//     }
//   }, [packageData, isLoading, error, pdfGenerated]);
  
//   const generatePDF = () => {
//     if (pdfGenerated) return; // Prevent multiple PDF generation
...
//     const captureAndAddElement = async (element: Element, yPosition: number): Promise<number> => {
//       return new Promise<number>((resolve) => {
//         html2canvas(element as HTMLElement, {
//           scale: 1.5, // Reduced from 2 to save size
//           useCORS: true,
...
//     const processPDF = async () => {
//       try {
//         setPdfGenerated(true); // Set flag to prevent multiple generation
...
//       <Box className="loading-container">
//         <p style={{ color: 'red' }}>
//           Error: {error || 'Failed to load booking data'}
//         </p>
...
//     if (!currentSearchParams?.rooms || !Array.isArray(currentSearchParams.rooms)) {
//       return {
//         totalRooms: parseInt(hotels?.[0]?.booking?.totalRooms?.toString() || '1') || 1,
//         totalAdults: Math.floor(parseInt(costs.packageDetails?.totalPersons?.toString() || '2') / 2) || 2,
//         totalChildren: 0,
//         totalCWB: 0,
//         totalCNB: parseInt(costs.packageDetails?.totalPersons?.toString() || '2') - Math.floor(parseInt(costs.packageDetails?.totalPersons?.toString() || '2') / 2) || 0,
//         totalInfants: 0
//       };
//     }
...
//           <Box className="booking-reference-content">
//             <Box className="booking-ref">
//               <span className="bold-text">BOOKING REFERENCE:</span> {refId}
//             </Box>
//             <Box className="generated-date">Generated on: {generateDate}</Box>
//             <Box className="client-name">Client Name: {leadData?.clientName || 'N/A'}</Box>
//             <Box className="status">Status: {leadData?.status || 'N/A'}</Box>
//           </Box>
//         </Box>
...
//                             return (
//                               <Box key={lineIndex} className="description-line bullet-point">
//                                 <span className="bullet">* </span>
//                                 <span className="bullet-content">{cleanLine}</span>
//                               </Box>
//                             );
//                           } else {
//                             return <div key={lineIndex} className="description-line">{line}</div>;
//                           }
//                         })}
//                     </Box>
...
//             <Box className="cost-label">Total Persons:</Box>
//             <Box className="cost-value">{totalPersons}</Box>
