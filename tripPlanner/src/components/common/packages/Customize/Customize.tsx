...
import { OptionType } from "../../../../types/types.ts";
...
import { CustomizeSearchProps, HotelSummaryParams } from "../../../../types/hotel.types.ts";
import { resetHotelsAndPlannerItems, selectTripPlannerState, setCurrentSearchParams, setTripPlannerParams } from "../../../../store/slices/tripPlannerSlice.ts";
...
const Customize: React.FC<CustomizeSearchProps> = ({ isModifying = false, initialValues = {}, onSearchComplete }) => {
  ...
  const getInitialSearchParams = () => {
    ...
    if (isModifying && tripPlannerState.currentSearchParams) {
      return tripPlannerState.currentSearchParams;
    }
    if (location.state) {
  ...
  const getInitialCountry = () => {
    if (searchParams?.country) {
      return countriesList.find(c => c.label === searchParams.country) || countriesList[0];
    }
  ...
  const [cityOptions, setCityOptions] = useState(
    citiesList.filter(city => city.countryId === country.id)
  );

  const getInitialCity = () => {
    if (searchParams?.city) {
      const foundCity = citiesList.find(c => c.label === searchParams.city);
  ...
  const [rooms, setRooms] = useState(
    searchParams?.rooms || [{ id: 1, adults: 2, cwb: 0, cnb: 0, infants: 0 }]
  );

  const { data, isLoading, error } = useGetHotelsByCityQuery({ 
  ...
  }, [isModifying, dispatch, tripPlannerState.currentSearchParams]);
  ...
  useEffect(() => {
    if (isModifying && country && city && checkInDate && checkOutDate) {
      ...
      dispatch(setCurrentSearchParams(currentParams));
    }
  }, [country, city, checkInDate, checkOutDate, nights, packageType, rooms, isModifying, dispatch]);
      ...
      <Container sx={{paddingLeft:'0rem', paddingRight:'0rem'}}>
        {!showHotelListing ? (
          <>
            {!isModifying && (
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography variant="h4" sx={{ color: '#333', fontWeight: 'bold' }} className="heading">
                  Customize Search
                </Typography>
            ...
            <Grid container spacing={1} className='form-container'>
                  ...
                  <Grid container spacing={2} className='select-form'>
                      ...
                      <FormControl fullWidth variant="outlined" size="small" className='select-option-1'>
                        <Autocomplete  
                          ...
                          sx={{
                            ...
                            '& .MuiInputBase-root': { height: '2rem', width: '16rem' }
                          }}
                        />
                      </FormControl>
                    </Grid>
                    ...
                    <Grid item xs={12}>
                      ...
                      >
                        <FormControlLabel 
                          value="hotel-land"  
                          control={<Radio size="small" sx={{height:'1rem',width:'1rem',padding:'1rem', '&.Mui-checked': { color: 'blue'} }}/>} 
                          label="Hotel + Land Package"  
                        />
                        <FormControlLabel 
                          value="land-only"  
                          control={<Radio size="small" sx={{height:'1rem',width:'1rem',padding:'1rem', '&.Mui-checked': { color: 'blue' } }}/>} 
                            ...
                            <Grid item xs={12} sm={2}>
                              ...
                              <FormControl fullWidth size="small">
                                <Select  
                                  ...
                                  onChange={(e) => handleRoomChange(room.id, 'adults', +e.target.value)}
                                  sx={{ height: '40px' }}  
                                >
                                  {[...Array(4)].map((_, i) => (
              ...
              <Grid item xs={12} md={5} className='side-image'>
                <Box sx={{ overflow: 'hidden', height: '100%' }} className='side-box'>
                  <img 
                    src="https://assets.micontenthub.com/traveloffers/travel-tips/baku_OIPJAbO-S.jpg"  
          ...
          <TripPlanner 
            ...
            checkInDate={checkInDate ? checkInDate.toISOString() : new Date().toISOString()}
            checkOutDate={checkOutDate ? checkOutDate.toISOString() : new Date().toISOString()} 
