...
import { cities, countries } from "../../../../model/selectOptions.ts";
...
const Customize: React.FC<CustomizeSearchProps> = ({ isModifying = false, initialValues = {}, onSearchComplete }) => {
  ...
  const findCountryOption = (countryName) => {
    if (!countryName) return null;
    const option = countries.find(c => c.label === countryName || c.value === countryName);
    return option || null;
  };
  
  const findCityOption = (cityName) => {
    if (!cityName) return null;
    const option = cities.find(c => c.label === cityName || c.value === cityName);
    return option || null;
  };
  
  const [showHotelListing, setShowHotelListing] = useState(false);
  const [country, setCountry] = useState<{ label: string; value: string } | null>(
    findCountryOption(searchParams?.country)
  );
  const [city, setCity] = useState<{ label: string; value: string } | null>(
    findCityOption(searchParams?.city)
  );
  
  // Parse dates properly - ensure we have valid Date objects
  ...
  // Calculate nights properly based on the actual dates
  ...
  const handleCheckInChange = (date: Date | null) => {
    ...
    if (checkOutDate && date >= checkOutDate) {
      // If check-in date is on or after check-out date, adjust check-out date
                ...
                <Grid container spacing={2} className='select-form'>
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth variant="outlined" size="small" className='select-option-1'>
                      <Autocomplete 
                        className="destination-select"  options={countries}
                        getOptionLabel={(option) => option.label} value={country} onChange={(_, newValue) => handleCountrySelect(newValue)}
                        renderInput={(params) => (
                        <TextField className='select-destination'  {...params} label="Select Country"  variant="outlined" size="small" fullWidth  required /> )}  sx={{bgcolor:'white','& .MuiInputBase-root': { height: '2rem', width:'16rem' } }}   />
                    </FormControl>
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <FormControl fullWidth variant="outlined" size="small" className='select-option-2'>
                      <Autocomplete  className="city-select" options={cities}
                        getOptionLabel={(option) => option.label}value={city} onChange={(_, newValue) => setCity(newValue)} renderInput={(params) => (
                          <TextField 
                            className='city-select' {...params}  label="Select City" variant="outlined" size="small" fullWidth  required />)} 
                            sx={{bgcolor:'white','& .MuiInputBase-root': { height: '2rem', width:'16rem' } }}  />
                    </FormControl>
                  </Grid>
                  {showPackageOptions && (
