...
const TripPlanner: React.FC<TripPlannerProps> = ({ nights, checkInDate, checkOutDate, onProceed }) => {
      ...
      try {
        ...
        setHotels(prevHotels => {
          ...
          if (src === 'editLead' || src === 'lead') {
            ...
            const updatedData = existingData.concat(existingHotels);
            sessionStorage.setItem('updatingHotelData', JSON.stringify(updatedData));
          }
          else if (src === 'B.E.') {
            const existingDataStr = sessionStorage.getItem('tripPlannerHotels');
            const existingData = existingDataStr ? JSON.parse(existingDataStr) : [];
            const updatedData = existingData.concat(existingHotels);
            sessionStorage.setItem('tripPlannerHotels', JSON.stringify(updatedData));
          }
          else {
            const existingDataStr = sessionStorage.getItem('tripPlannerHotels');
            const existingData = existingDataStr ? JSON.parse(existingDataStr) : [];
    ...
    if (tourData) {
      try {
        ...
        setPlannerItems(prevItems => {
          ...
          // let src = sessionStorage.getItem('src');
          // if(src === 'editLead' || src === 'lead'){
          //   const existingItemsStr = sessionStorage.getItem('updatingtripPlannerItems');
          //   const existingItems = existingItemsStr ? JSON.parse(existingItemsStr) : [];
          //   const combinedItems = existingItems.concat(updatedItems);
          //   sessionStorage.setItem('updatingtripPlannerItems', JSON.stringify(combinedItems));
          // }
          // else{
  ...
  useEffect(() => {
    ...
    if (src === 'lead' || src === 'editLead') {
      let itemDataToUpdate: any = null;
      savedItemData = sessionStorage.getItem('editLeadData');
      if (savedItemData !== null) {
        itemDataToUpdate = JSON.parse(savedItemData);
      }
      savedItems = itemDataToUpdate.plannerItems;
    }
    else {
      savedItemData = sessionStorage.getItem('tripPlannerItems');
      if (savedItemData !== null) {
        savedItems = JSON.parse(savedItemData);
      }
    }
  ...
  useEffect(() => {
    ...
    let src = sessionStorage.getItem('src');
    if (src === 'lead') {
      let hotelDataToUpdate = sessionStorage.getItem('tripPlannerHotels');
      if (hotelDataToUpdate !== null) {
        sessionStorage.setItem('updatingHotelData', hotelDataToUpdate);
      }
      savedHotels = sessionStorage.getItem('updatingHotelData');
    }
    else if (src === 'editLead') {
      savedHotels = sessionStorage.getItem('updatingHotelData');
    }
    else {
      savedHotels = sessionStorage.getItem('tripPlannerHotels');
    }
  ...
  const handleClientFormSubmit = async (clientData) => {
    ...
    const mergedHotels = [
      ...(editData.hotelDetails || editData.hotels || []),
      ...(hotels || [])
    ];
    ...
    sessionStorage.setItem('editLeadData', JSON.stringify({
      ...editData,
      hotels: mergedHotels,
      plannerItems: mergedPlannerItems
    }));
    ...
    <Box className="trip-planner-page">
          ...
          <>
            ...
            <Box sx={{ mb: 0, height: '2rem' }} className='tablist-container'>
              <Tabs className='tablist-btn' value={activeTab} onChange={handleTabChange} sx={{ color: 'black' }}>
                <Tab className='planner-btn' value="planner" label="Planner" style={{ color: activeTab === 'planner' ? 'white' : 'black', }} sx={{
                  color: activeTab === 'planner' ? 'black' : 'white',
                  bgcolor: activeTab === 'planner' ? 'grey' : 'white', marginLeft: '0rem', width: '10rem'
                }} />
                {showHotelTab && (
                  <Tab className='hotel-btn' value="hotel" label="Hotel Details" style={{ color: activeTab === 'planner' ? 'black' : 'white', }} sx={{
                    color: activeTab === 'planner' ? 'black' : 'white',
                    ...
                    <Box className="cell">
                      {plannerItem.hotel ? (
                        <Box className="selected-hotel">
                          <Box sx={{}}>
                            <Typography className='hotel_name' variant="body2" sx={{
      ...
      {clientFormOpen && (
        ...
        <ClientDetailsForm
          ...
          activities={[]} persons={''}
          selectedHotels={hotels} // Pass current hotel selections
