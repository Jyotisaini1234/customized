...
const TripPlanner: React.FC<TripPlannerProps> = ({ nights, checkInDate, checkOutDate, onProceed,initialClientData = null }) => {
  ...
  const generateInitialPlannerItems = () => {
    ...
    for (let i = 0; i <= totalNights; i++) {
      ...
      items.push({ id: `day-${items.length + 1}`, date: `${day.toString().padStart(2, '0')}-${month} ${year}`, dateObj: new Date(dayDate),  hotel: null, transfer: null,tours: null,  meals: null,eventDate: '',}); currentDate.setDate(currentDate.getDate() + 1);
    }
    return items;
  };
