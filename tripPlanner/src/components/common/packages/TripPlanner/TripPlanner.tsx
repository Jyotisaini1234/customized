...
const TripPlanner: React.FC<TripPlannerProps> = ({ nights, checkInDate, checkOutDate, onProceed, initialClientData = null }) => {
  ...
  const generateInitialPlannerItems = useCallback((searchParams = currentSearchParams) => {
    ...
    return items;
  }, [checkInDate, checkOutDate, currentSearchParams]);
  ...
  useEffect(() => {
    ...
    const initializePlannerItems = () => {
      ...
      if (src === 'lead' || src === 'editLead') {
        ...
        if (savedItemData !== null) {
          ...
          if (itemDataToUpdate.plannerItems && itemDataToUpdate.plannerItems.length > 0) {
        ...
        if (storedEditData) {
          try {
            ...
            if (parsedData.isEditMode) {
              setIsEditMode(true);
              setEditingClientData({  ...parsedData, plannerItems: parsedData.plannerItems || [],  hotelDetails: parsedData.hotelDetails || []  });
              setOriginalLeadId(parsedData.leadId || parsedData.bookingRef || null);
              setCurrentSearchParams(parsedData.currentSearchParams || {});
              if (Array.isArray(parsedData.hotelDetails) || Array.isArray(parsedData.hotels)) {
                setHotels(parsedData.hotelDetails || parsedData.hotels || []);
              }
              if (Array.isArray(parsedData.plannerItems) || Array.isArray(parsedData.tours)) {
                const itemsToSet = parsedData.plannerItems || parsedData.tours || [];
                if (itemsToSet.length > 0) { setPlannerItems(itemsToSet); } }
              return true;
            }
    ...
    const editModeInitialized = initializeEditMode();
    if (!editModeInitialized) {
      initializeHotels();
      initializePlannerItems();
    }
    ...
    if (tourData) {
      try {
        ...
        setPlannerItems(prevItems => {
          ...
          const targetIndex = updatedItems.findIndex(item => item.id === tourDetails.specificDayId);
          if (targetIndex !== -1) {
            updatedItems[targetIndex] = {
              ...updatedItems[targetIndex],
              tours: { name: tourDetails.tour.tourName,details: tourDetails}
            };
            console.log('Updated tour for day:', updatedItems[targetIndex]); 
          } else { 
    ...
    if ((src === 'editLead' || src === 'lead') && !hotelData) {
      ...
      if (updatingHotelData) {
        try {
          ...
          if (Array.isArray(sessionHotels) && sessionHotels.length > 0) {
            setHotels(prevHotels => {
              if (sessionHotels.length !== prevHotels.length) {
                return sessionHotels;}
              return prevHotels;
            });
          }
        } catch (e) { console.error('Error loading session hotel data', e); }
      }
      ...
      if (updatingTripPlannerItems) {
        try {
              ...
              sessionItems.forEach(sessionItem => {
                ...
                if (matchingIndex !== -1 && sessionItem.tours && !mergedItems[matchingIndex].tours) {
                  mergedItems[matchingIndex] = {
                    ...mergedItems[matchingIndex], tours: sessionItem.tours};  
                }
              });
              return mergedItems;
            });  
          ...
          console.error('Error parsing trip planner items:', e);
        }
      }
    }
    setEditDataVersion(prev => prev + 1);
  }, []);

  useEffect(() => {
    const src = sessionStorage.getItem('src');
    const calculateTotalsOptimized = () => {
      let hotelTotal = 0;
      ...
      const newGrandTotal = hotelTotal + tourTotal;
      setGrandTotal(prevTotal => {
        if (Math.abs(prevTotal - newGrandTotal) > 0.01) {
          console.log(`Total calculation - Hotels: ${hotelTotal}, Tours: ${tourTotal}, Grand Total: ${newGrandTotal}`);
          return newGrandTotal; }
        return prevTotal;
      });
      if (hotels.length > 0) {
        const hotelCurrency = hotels[0]?.booking?.currency || 'USD';
        setCurrency(prevCurrency => prevCurrency !== hotelCurrency ? hotelCurrency : prevCurrency);
        if (!selectedHotel) {
    ...
    const assignHotelsToPlanner = () => {
      if (hotels.length > 0) {
            ...
            if (hotel.specificDayId) {
              ...
              if (dayIndex !== -1) {
                updatedItems[dayIndex] = {
                  ...updatedItems[dayIndex],
                  hotel: {name: hotel.hotel?.hotelName || "Unknown Hotel", details: hotel, hotelSpecificDetails: hotel.hotelSpecificDetails || {}   }
                };
                
                if (dayIndex === totalDays - 2) {
                  const lastDayIndex = totalDays - 1;
                  updatedItems[lastDayIndex] = {
                    ...updatedItems[lastDayIndex], hotel: { name: hotel.hotel?.hotelName || "Unknown Hotel",details: hotel, hotelSpecificDetails: hotel.hotelSpecificDetails || {} } 
                  };
                }
              }
              ...
              if (checkInDayIndex !== -1) {
                ...
                for (let i = startShowIndex; i <= endShowIndex; i++) {
                  updatedItems[i] = {
                    ...updatedItems[i],
                    hotel: {name: hotel.hotel?.hotelName, details: hotel, hotelSpecificDetails: hotel.hotelSpecificDetails || {} }
                  };
                }
              }
              ...
              updatedItems.forEach((item, index) => {
                ...
                if (itemDate >= checkInDate && itemDate < checkOutDate) {
                  updatedItems[index].hotel = { name: hotel.hotel?.hotelName || "Unknown Hotel", details: hotel, hotelSpecificDetails: hotel.hotelSpecificDetails || {} }; 
                }
              });
            }
          });
        ...
        setPlannerItems(prevItems => prevItems.map(item => ({ ...item, hotel: null })));
      }
    };

    const timeoutId = setTimeout(() => {
      calculateTotalsOptimized();
      assignHotelsToPlanner();
    }, 150);
    
    return () => clearTimeout(timeoutId);

  }, [
    hotels.map(h => h.uniqueId + (h.booking?.totalPrice || 0)).join(','),
    plannerItems.map(p => p.id + (p.tours?.price || p.tours?.details?.booking?.totalPrice || 0)).join(','),
    currentSearchParams.nights
  ]);
  const handleHotelSelection = (itemId) => {
    let src = sessionStorage.getItem('src');
    if (src === 'lead') {
      sessionStorage.setItem('src', 'editLead'); 
    }
  ...
  const handleAddItem = (itemId: string, itemType: 'tours' ) => {
    ...
    if (itemType === 'tours') {
      ...
      if (currentSearchParams.rooms && currentSearchParams.rooms.length > 0) {
        params.append('rooms', decodeURIComponent(JSON.stringify(currentSearchParams.rooms)));  
      }
      
      window.location.href = `${TRIP_PLANNER}${params.toString()}`;
    } else {
      navigate(`/${itemType}-summary`, { 
        state: {...currentSearchParams,dayId: itemId.split('-')[1], fromTripPlanner: true, checkInDate: itemDate.toISOString(),  city: selectedCity,   country: selectedCountry }  });
    }
  };
  ...
  const handleRemoveHotel = (plannerItem) => {
    ...
    if (src === 'editLead' || src === 'lead') { 
      sessionStorage.setItem('updatingHotelData', JSON.stringify(updatedHotels)); 
    } else if (src === 'B.E.') { 
      sessionStorage.setItem('tripPlannerHotels', JSON.stringify(updatedHotels)); 
    } else { 
      sessionStorage.setItem('tripPlannerHotels', JSON.stringify(updatedHotels)); 
  ...
  const displayNights = currentSearchParams?.nights || nights ;
  ...
  const handleSearchComplete = (updatedParams: any) => {
    if (updatedParams) {
      ...
      const generateUpdatedPlannerItems = () => {
        let startDate, endDate;
        if (updatedParams.checkInDate && updatedParams.checkOutDate) {
          startDate = new Date(updatedParams.checkInDate);
          endDate = new Date(updatedParams.checkOutDate);
        } else {
          startDate = new Date();
          endDate = new Date();
          endDate.setDate(endDate.getDate() + 1);
        ...
        if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
          console.error('Invalid date values in updated params');
          startDate = new Date();
          endDate = new Date();
          endDate.setDate(endDate.getDate() + 1);
        }
      ...
      const newPlannerItems = generateUpdatedPlannerItems();
      setPlannerItems(newPlannerItems);
      ...
      if (hotels.length > 0) {
        ...
        const src = sessionStorage.getItem('src');
        if (src === 'editLead' || src === 'lead') {
          sessionStorage.setItem('updatingHotelData', JSON.stringify(filteredHotels));
        } else {
          sessionStorage.setItem('tripPlannerHotels', JSON.stringify(filteredHotels));
        }
      }
      setGrandTotal(0);
      setMarginTotal('0');
  ...
  const handleClientFormSubmit = async ({name, options, hotels: incomingHotels,  plannerItems: incomingPlannerItems,  marginTotal, grandTotal, bookingRef,  travelDate, currency,  bookingStatus,  destination,...restClientData}) => {
    ...
    const calculateRoomCounts = () => {
      ...
      if (currentSearchParams?.rooms && Array.isArray(currentSearchParams.rooms)) {
        currentSearchParams.rooms.forEach(room => {  
          totalRooms += 1; 
          adult += room.adults || 0; 
          cnb += room.cnb || 0;
          cwb += room.cwb || 0; 
        ...
        totalRooms = editData.totalRooms || 1;
        adult = editData.adult || 2;
        cnb = editData.cnb || 0; 
        cwb = editData.cwb || 0; 
      }
    ...
    const calculateTotalPersons = () => { if (!currentSearchParams?.rooms || !Array.isArray(currentSearchParams.rooms)) {return editData.totalPersons || 2;  }
    ...
    const newLead = {
      ...
      plannerItems: finalPlannerItems.map(item => {
        ...
        const getDescription = (tourItem: any) => {
          ...
          for (const desc of possibleDescriptions) { if (desc && desc.trim().length > 0) { return desc; } }
        ...
        return {
          ...
          tours: item.tours ? {
            ...
            duration: item.tours.details?.tour?.eventDuration || item.tours.duration || 'N/A',
  ...
  const isCheckInDate = (plannerItem, hotels) => {
    if (!plannerItem.hotel) return false;
    const hotelDetails = plannerItem.hotel.details;
    if (!hotelDetails) return false;
    const itemDate = new Date(plannerItem.dateObj);
    const hotelCheckInDate = new Date(hotelDetails.booking?.checkInDate);
    itemDate.setHours(0, 0, 0, 0);
    hotelCheckInDate.setHours(0, 0, 0, 0);
    return itemDate.getTime() === hotelCheckInDate.getTime();
  };

  const canHaveHotel = (plannerItem, plannerItems) => {
    const currentIndex = plannerItems.findIndex(item => item.id === plannerItem.id);
    const totalDays = plannerItems.length;
    return currentIndex < totalDays - 1;
  };
...
return (
    <Box className="trip-planner-page">
      <Container sx={{ paddingLeft: '0rem', paddingRight: '0rem', maxWidth: '100%' }}>
                    ...
                    <Box className="cell">
                      {plannerItem.hotel ? (
                        <Box className="selected-hotel">
                          <Box><Typography className='hotel_name' variant="body2" sx={{fontSize: '0.8rem', textOverflow: 'ellipsis',    whiteSpace: 'nowrap',  overflow: 'hidden', textAlign: 'center', maxWidth: '100%',  }}>{plannerItem.hotel.name}</Typography>  </Box>
                          {isCheckInDate(plannerItem, hotels) && (
                            <Box>
                        ...
                        canHaveHotel(plannerItem, plannerItems) && (
        ...
        {activeTab === 'hotel' && showHotelTab && !showThankYou && ( <Paper elevation={3} className="hotel-details-container" sx={{ bgcolor: 'transparent', boxShadow: 'none' }}> <TripDetails hotels={hotels} totalPrice={undefined} /></Paper> )}
        {showThankYou && (
